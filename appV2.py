...
def generate_resume_with_reportlab(data):
    ...
    # Add Hobby (New Section)
    if data.get("hobbies"):
        hobbies = [f"- {hobby}" for hobby in data["hobbies"]]
...
if "hobbies" not in st.session_state:
    st.session_state.hobbies = []  # For hobbies
...
# Section: Hobbies (Newly Added)
st.header("🎯 Hobbies")
for i, hobby in enumerate(st.session_state.hobbies):
    hobby_input = st.text_input(f"Hobby {i + 1}", value=hobby, key=f"hobby_{i}")
    st.session_state.hobbies[i] = hobby_input
    if st.button(f"Remove Hobby {i + 1}", key=f"remove_hobby_{i}"):
        remove_hobby(i)
        st.experimental_rerun()
...
if st.button("Generate Resume"):
    resume_data = {
        ...
        "phone": phone,
        "summary": summary,
        "skills": {skill["name"]: skill["score"] for skill in st.session_state.skills},
        "education": st.session_state.educations,
        "work_experience": st.session_state.work_experiences,
        "projects": st.session_state.projects,
        "hobbies": st.session_state.hobbies,  # Add hobbies data
    }

    pdf = generate_resume_with_reportlab(resume_data)
